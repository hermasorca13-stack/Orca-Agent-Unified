...
def get_registry() -> SkillRegistry:
    ...
    def _register_termux_skills(self):
        """Termux and Android automation skills"""
        from core.skills.termux_automation import TermuxAutomationSkills
        termux_skills_instance = TermuxAutomationSkills()

        @self.register_function(
            name="adb_command",
            description="Executes an arbitrary ADB shell command on the connected Android device.",
            category=SkillCategory.SYSTEM,
            parameters={
                "type": "object",
                "properties": {
        ...
        async def adb_command(command: str) -> str:
            return await termux_skills_instance.adb_command(command)

        @self.register_function(
            name="adb_tap",
            description="Simulates a tap event on the Android device screen at the specified coordinates.",
            category=SkillCategory.SYSTEM,
            parameters={
                "type": "object",
                "properties": {
        ...
        @self.register_function(
            name="adb_text",
            description="Inputs text into the active element on the Android device.",
            category=SkillCategory.SYSTEM,
            parameters={
                "type": "object",
                "properties": {
        ...
        @self.register_function(
            name="adb_swipe",
            description="Simulates a swipe gesture on the Android device screen.",
            category=SkillCategory.SYSTEM,
            parameters={
                "type": "object",
                "properties": {
        ...
        @self.register_function(
            name="adb_screenshot",
            description="Takes a screenshot and saves it to the specified path on the Android device.",
            category=SkillCategory.SYSTEM,
            parameters={
                "type": "object",
                "properties": {
                    "output_path": {"type": "string", "description": "Path on the Android device to save the screenshot"}
                },
                "required": []
            }
        )
        async def adb_screenshot(output_path: str = "/sdcard/screenshot.png") -> str:
            return await termux_skills_instance.adb_screenshot(output_path)

        @self.register_function(
            name="adb_keyevent",
            description="Sends a key event to the Android device.",
            category=SkillCategory.SYSTEM,
            parameters={
                "type": "object",
                "properties": {
                    "keycode": {"type": "integer", "description": "The keycode to send (e.g., 4 for back, 66 for enter)"}
                },
                "required": ["keycode"]
            }
        )
