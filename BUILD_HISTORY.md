...
- `PYTHONPATH=. pytest -q tests/trading_bot` — **23 passed** after section 20 integration, including Meta-Labeling, CPCV/PBO/DSR, shadow, retirement, execution feedback, and unified orchestration tests.
...
- Full repository suite with the three external quota tests deselected — **633 passed, 4 skipped, 3 deselected**. The three deselected failures are pre-existing OpenAI quota/API tests (`DALL-E` and `Whisper`) caused by an exhausted external account quota, not by the trading addition. The initial Telegram collection mismatch was repaired additively through the existing adapter compatibility layer.
...
- Section 20 real-data Meta-Labeling verification — 936 Triple-Barrier events from 1000 Binance 1h bars, split into 655 training and 281 test events; mean test probability 0.518 and acceptance rate 44.84%. This is a filter diagnostic, not a claimed Win Rate improvement, and it is not promoted to Live.
- Added beta-weighted hedge planning, position lifecycle management, validation tooling, staged ML model registry approval, durable market storage, live-data hub, a systemd daemon template, local OS-keyring API setup, cumulative parameter optimization with promotion gates, real-data reports, a Windows-native installation/local-key setup/Scheduled Task path, and section 20 adaptive components: Meta-Labeling/Triple-Barrier, regime detection, dynamic allocation, CPCV/PBO/Deflated Sharpe, execution feedback, shadow gates, retirement, Fractional Kelly, and unified feedback orchestration. A pre-Windows Git tag and archive backup were created before modification. The expanded ORCA test suite passed **23/23**; the optimized SMA reached OOS Win Rate **44.78%** and remained correctly **rejected** for live use, while the 100% crossover result was rejected because it had only two OOS trades.
