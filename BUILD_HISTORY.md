...
- Added beta-weighted hedge planning, position lifecycle management, validation tooling, staged ML model registry approval, durable market storage, live-data hub, a systemd daemon template, local OS-keyring API setup, cumulative parameter optimization with promotion gates, real-data reports, and a Windows-native installation/local-key setup/Scheduled Task path. A pre-Windows Git tag and archive backup were created before modification. The expanded ORCA test suite passed **18/18**; the optimized SMA reached OOS Win Rate **44.78%** and remained correctly **rejected** for live use, while the 100% crossover result was rejected because it had only two OOS trades.
